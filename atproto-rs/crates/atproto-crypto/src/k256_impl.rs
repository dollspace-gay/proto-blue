...
/// A secp256k1 (K-256) ECDSA keypair.
///
...
impl K256Keypair {
    ...
    pub fn verifier_from_compressed(compressed: &[u8]) -> Result<K256Verifier, CryptoError> {
        ...
        if public_key.is_none().into() {
            return Err(CryptoError::InvalidKey(
                "Invalid secp256k1 compressed public key".to_string(),
            ));
        }
        let verifying_key = VerifyingKey::from(public_key.unwrap());
        Ok(K256Verifier { verifying_key })
    }
}
...
impl keypair::Keypair for K256Keypair {
    fn did(&self) -> String {
        let compressed = self.public_key_compressed();
        format_did_key("ES256K", &compressed)
    }
...
impl keypair::ExportableKeypair for K256Keypair {
    fn export_private_key(&self) -> Vec<u8> {
        self.signing_key.to_bytes().to_vec()
    }
}

/// A secp256k1 public key verifier (no private key).
pub struct K256Verifier {
    verifying_key: VerifyingKey,
}

impl keypair::Verifier for K256Verifier {
...
pub fn compress_pubkey(uncompressed: &[u8]) -> Result<Vec<u8>, CryptoError> {
    ...
    if pk.is_none().into() {
        return Err(CryptoError::InvalidKey(
            "Invalid secp256k1 point".to_string(),
        ));
    }
...
mod tests {
    ...
    fn generate_and_sign_verify() {
        ...
        assert_eq!(
            sig.len(),
            64,
            "secp256k1 compact signature should be 64 bytes"
        );
    ...
    fn export_reimport() {
        ...
        let kp2 = K256Keypair::from_private_key(&private_bytes).unwrap();
