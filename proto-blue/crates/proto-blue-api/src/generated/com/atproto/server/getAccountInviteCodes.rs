    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("com.atproto.server.getAccountInviteCodes", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        create_available: ctx.params.get("createAvailable").and_then(|v| v.parse::<bool>().ok()),
        include_used: ctx.params.get("includeUsed").and_then(|v| v.parse::<bool>().ok()),
    })
}
