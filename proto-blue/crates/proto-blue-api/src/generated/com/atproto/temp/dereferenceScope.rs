...
fn map_xrpc_error(err: proto_blue_xrpc::XrpcError) -> CallError {
    match err.error.as_deref() {
        Some("InvalidScopeReference") => CallError::InvalidScopeReference,
...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    { let v = &p.scope; qp.insert("scope".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("com.atproto.temp.dereferenceScope", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("com.atproto.temp.dereferenceScope", move |ctx| {
