...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    if let Some(v) = &p.birth_date { qp.insert("birthDate".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
    if let Some(v) = &p.email { qp.insert("email".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
...
pub async fn call(
    ...
    let response = match client.query("com.atproto.temp.checkHandleAvailability", qp.as_ref(), opts).await {
        Ok(r) => r,
