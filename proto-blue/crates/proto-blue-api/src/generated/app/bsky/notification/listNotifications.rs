    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.notification.listNotifications", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        ...
        priority: ctx.params.get("priority").and_then(|v| v.parse::<bool>().ok()),
        reasons: Some(ctx.params.get("reasons").map(|v| v.split(',').map(String::from).collect::<Vec<_>>()).unwrap_or_default()),
