...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    { let v = &p.country_code; qp.insert("countryCode".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
    if let Some(v) = &p.region_code { qp.insert("regionCode".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.ageassurance.getState", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.ageassurance.getState", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        country_code: (ctx.params.get("countryCode").cloned())?,
        region_code: ctx.params.get("regionCode").cloned(),
    })
}
