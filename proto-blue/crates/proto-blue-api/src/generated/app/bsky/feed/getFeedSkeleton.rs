...
fn map_xrpc_error(err: proto_blue_xrpc::XrpcError) -> CallError {
    match err.error.as_deref() {
        Some("UnknownFeed") => CallError::UnknownFeed,
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.feed.getFeedSkeleton", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.feed.getFeedSkeleton", move |ctx| {
