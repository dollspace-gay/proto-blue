...
fn map_xrpc_error(err: proto_blue_xrpc::XrpcError) -> CallError {
    match err.error.as_deref() {
        Some("UnknownList") => CallError::UnknownList,
...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    { let v = &p.list; qp.insert("list".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.feed.getListFeed", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.feed.getListFeed", move |ctx| {
