...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    if let Some(v) = &p.algorithm { qp.insert("algorithm".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.feed.getTimeline", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.feed.getTimeline", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        algorithm: ctx.params.get("algorithm").cloned(),
