...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    { let v = &p.feeds; qp.insert("feeds".to_string(), proto_blue_xrpc::QueryValue::Array(v.iter().map(|x| proto_blue_xrpc::QueryValue::String(x.clone())).collect())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.feed.getFeedGenerators", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.feed.getFeedGenerators", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        feeds: (Some(ctx.params.get("feeds").map(|v| v.split(',').map(String::from).collect::<Vec<_>>()).unwrap_or_default()))?,
    })
}
