...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    if let Some(v) = &p.typeahead { qp.insert("typeahead".to_string(), proto_blue_xrpc::QueryValue::Boolean(*v)); }
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.unspecced.searchActorsSkeleton", qp.as_ref(), opts).await {
        Ok(r) => r,
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        ...
        typeahead: ctx.params.get("typeahead").and_then(|v| v.parse::<bool>().ok()),
