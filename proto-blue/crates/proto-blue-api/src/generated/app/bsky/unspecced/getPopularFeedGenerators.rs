...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    if let Some(v) = &p.query { qp.insert("query".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.unspecced.getPopularFeedGenerators", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.unspecced.getPopularFeedGenerators", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        ...
        query: ctx.params.get("query").cloned(),
    })
}
