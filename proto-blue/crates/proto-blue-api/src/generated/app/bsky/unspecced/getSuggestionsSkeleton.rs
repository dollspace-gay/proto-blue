...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    if let Some(v) = &p.relative_to_did { qp.insert("relativeToDid".to_string(), proto_blue_xrpc::QueryValue::String(v.clone())); }
...
pub async fn call(
    ...
    let response = match client.query("app.bsky.unspecced.getSuggestionsSkeleton", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("app.bsky.unspecced.getSuggestionsSkeleton", move |ctx| {
