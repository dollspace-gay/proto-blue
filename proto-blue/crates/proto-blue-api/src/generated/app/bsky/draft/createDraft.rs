...
fn map_xrpc_error(err: proto_blue_xrpc::XrpcError) -> CallError {
    match err.error.as_deref() {
        Some("DraftLimitReached") => CallError::DraftLimitReached,
...
pub async fn call(
    ...
    let response = match client.procedure("app.bsky.draft.createDraft", qp_ref, Some(body), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.procedure("app.bsky.draft.createDraft", move |ctx| {
