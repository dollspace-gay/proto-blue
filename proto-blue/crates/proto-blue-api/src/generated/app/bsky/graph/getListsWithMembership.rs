...
pub async fn call(
    ...
    let response = match client.query("app.bsky.graph.getListsWithMembership", qp.as_ref(), opts).await {
        Ok(r) => r,
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        ...
        purposes: Some(ctx.params.get("purposes").map(|v| v.split(',').map(String::from).collect::<Vec<_>>()).unwrap_or_default()),
    })
}
