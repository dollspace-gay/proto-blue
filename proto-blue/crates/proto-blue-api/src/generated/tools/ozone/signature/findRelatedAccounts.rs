...
pub async fn call(
    ...
    let response = match client.query("tools.ozone.signature.findRelatedAccounts", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("tools.ozone.signature.findRelatedAccounts", move |ctx| {
