...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    { let v = &p.values; qp.insert("values".to_string(), proto_blue_xrpc::QueryValue::Array(v.iter().map(|x| proto_blue_xrpc::QueryValue::String(x.clone())).collect())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("tools.ozone.signature.searchAccounts", qp.as_ref(), opts).await {
        Ok(r) => r,
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        ...
        values: (Some(ctx.params.get("values").map(|v| v.split(',').map(String::from).collect::<Vec<_>>()).unwrap_or_default()))?,
    })
}
