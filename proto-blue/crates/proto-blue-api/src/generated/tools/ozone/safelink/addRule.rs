...
fn map_xrpc_error(err: proto_blue_xrpc::XrpcError) -> CallError {
    match err.error.as_deref() {
        Some("InvalidUrl") => CallError::InvalidUrl,
        Some("RuleAlreadyExists") => CallError::RuleAlreadyExists,
...
pub async fn call(
    ...
    let response = match client.procedure("tools.ozone.safelink.addRule", qp_ref, Some(body), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.procedure("tools.ozone.safelink.addRule", move |ctx| {
