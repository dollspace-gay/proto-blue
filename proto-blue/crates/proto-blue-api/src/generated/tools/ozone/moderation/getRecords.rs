...
pub async fn call(
    ...
    let response = match client.query("tools.ozone.moderation.getRecords", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("tools.ozone.moderation.getRecords", move |ctx| {
...
fn params_from_ctx(ctx: &proto_blue_xrpc::HandlerContext) -> Option<Params> {
    ...
    Some(Params {
        uris: (Some(ctx.params.get("uris").map(|v| v.split(',').map(String::from).collect::<Vec<_>>()).unwrap_or_default()))?,
    })
}
