...
fn to_query_params(p: &Params) -> proto_blue_xrpc::QueryParams {
    ...
    { let v = &p.subjects; qp.insert("subjects".to_string(), proto_blue_xrpc::QueryValue::Array(v.iter().map(|x| proto_blue_xrpc::QueryValue::String(x.clone())).collect())); }
    qp
}
...
pub async fn call(
    ...
    let response = match client.query("tools.ozone.moderation.getSubjects", qp.as_ref(), opts).await {
        Ok(r) => r,
    ...
    Fut: std::future::Future<Output = Result<Output, proto_blue_xrpc::XrpcServerError>> + Send + 'static,
{
    ...
    server.query("tools.ozone.moderation.getSubjects", move |ctx| {
