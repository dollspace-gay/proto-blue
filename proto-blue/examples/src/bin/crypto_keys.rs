...
//!
//! Run with: cargo run -p proto-blue-examples --bin `crypto_keys`
...
fn main() {
    ...
    let p256_key = proto_blue_crypto::P256Keypair::generate();
    ...
    let verifier = proto_blue_crypto::P256Keypair::verifier_from_compressed(&compressed).unwrap();
    ...
    println!("  Tampered verification: {wrong_valid} (expected false)");

    // --- K-256 (ES256K / secp256k1) keypair ---
    println!("\n--- K-256 (ES256K / secp256k1) Keypair ---");
    let k256_key = proto_blue_crypto::K256Keypair::generate();
    let k_compressed = k256_key.public_key_compressed();
    let k_did_key = proto_blue_crypto::format_did_key("ES256K", &k_compressed);
    ...
    // --- SHA-256 ---
    println!("\n--- SHA-256 ---");
    let hash = proto_blue_crypto::sha256(b"Hello, world!");
    println!("  SHA-256('Hello, world!'): {}", {
        let mut s = String::with_capacity(hash.len() * 2);
        for b in &hash {
            write!(s, "{b:02x}").expect("infallible write to String");
        }
        s
    });
